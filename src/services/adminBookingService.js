 ...
 */
const extractBookingsArray = (data) => {
 ...
 */
const extractSingleBooking = (data) => {
 ...
 */
export const updateBookingByAdmin = async (id, updates) => {
 ...
 */
export const updateBookingPaymentStatusAdmin = async (id, paymentStatus) => {
    const res = await axios.patch(`${API_URL}/admin/bookings/${id}/payment-status`, { paymentStatus }, getAuthHeaders());
    return extractSingleBooking(res.data);
};
/**
 ...
 */
export const filterBookingsByDateRange = async (from, to) => {
    const res = await axios.get(`${API_URL}/admin/bookings/filter/date?from=${from}&to=${to}`, getAuthHeaders());
    return extractBookingsArray(res.data);
};
/**
 * Filters bookings by booking status and/or payment status (admin only).
 */
export const filterBookingsByStatus = async (status, paymentStatus) => {
    const params = new URLSearchParams();
    if (status)
        params.append('status', status);
    if (paymentStatus)
        params.append('paymentStatus', paymentStatus);
    const res = await axios.get(`${API_URL}/admin/bookings/filter/status?${params.toString()}`, getAuthHeaders());
    return extractBookingsArray(res.data);
};
/**
 * Filters bookings by type: 'property' or 'tour' (admin only).
 */
export const filterBookingsByType = async (type) => {
    const res = await axios.get(`${API_URL}/admin/bookings/filter/type?type=${type}`, getAuthHeaders());
    return extractBookingsArray(res.data);
};
/**
...
export const exportAdminBookingsToPDF = async (filters) => {
    ...
    const res = await axios.get(`${API_URL}/admin/bookings/export/pdf?${params.toString()}`, {
        ...getAuthHeaders(),
        responseType: 'blob',
    });
...
export const getBookingStats = async () => {
    try {
        ...
        const stats = res.data?.data?.stats || res.data?.stats || res.data;
        return stats;
    }
    catch (error) {
        console.warn('⚠️ Failed to fetch stats from backend. Falling back to local calculation.', error);
