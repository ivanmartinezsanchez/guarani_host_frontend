...
const normalizePublicTour = (raw) => {
    ...
    const firstName = typeof rawHost.firstName === 'string' ? rawHost.firstName : '';
    const lastName = typeof rawHost.lastName === 'string' ? rawHost.lastName : '';
    const email = typeof rawHost.email === 'string' ? rawHost.email : '';
    const host = {
 ...
 * }
 */
export const getPublicTours = async (filters = {}) => {
    try {
        ...
        const toursFromApi = res.data.tours || [];
        const total = res.data.total ?? toursFromApi.length;
        const page = res.data.page ?? 1;
        const limit = res.data.limit ?? toursFromApi.length;
        // Normalize every tour to a safe, typed object
        const tours = toursFromApi.map(normalizePublicTour);
        return {
            tours,
            total,
            page,
 ...
 */
export const searchTours = async (searchTerm, filters = {}) => {
    try {
...
//# sourceMappingURL=publicTourService.js.map